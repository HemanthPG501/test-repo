...
async function waitForValidation(page) {
  ...
  for (let attempt = 1; attempt <= 120; attempt++) {
    ...
    writeLog(
      `validation-attempt-${attempt}.txt`,
      bodyText
    );

    const validationFrame =
      await findFrameContainingText(
        page,
        /Review Changes Before Committing|Check complete|Errors found|Commit|Cancel/i
      );

    const errorsFound =
      /errors?\s+found/i.test(bodyText);

    if (errorsFound) {
      console.log(
        'Validation failed. Errors found on validation page.'
      );

      await screenshot(
        page,
        '10-validation-errors-found.png'
      );

      writeLog(
        '10-validation-errors-found.txt',
        bodyText
      );
      ...
      if (validationFrame) {
        try {
          console.log(
            'Clicking Cancel because validation errors were found...'
          );
          ...
          await screenshot(
            page,
            '10a-after-validation-error-cancel.png'
          );

          writeLog(
            '10a-after-validation-error-cancel.txt',
            await getFullPageText(page)
          );
        } catch (cancelError) {
          console.log(
            `Unable to click Cancel after validation error: ${cancelError.message}`
          );
        }
      }

      throw new Error(
        'Validation failed. Errors found. Cancel clicked and logs captured.'
      );
    }

    if (validationFrame) {
      const checkComplete =
        /check complete/i.test(bodyText);
      ...
      if (checkComplete && commitVisible) {
        console.log(
          'Validation completed successfully. Commit button is visible.'
        );

        await screenshot(
          page,
          '10-validation-complete-no-errors.png'
        );

        writeLog(
          '10-validation-complete-no-errors.txt',
          bodyText
        );
  ...
  await screenshot(
    page,
    '10-validation-timeout.png'
  );

  writeLog(
    '10-validation-timeout.txt',
    await getFullPageText(page)
  );

  throw new Error(
    'Timed out waiting for validation completion.'
  );
}
...
async function extractDeploymentLog(page, commitFrame, fileName) {
  console.log(
    'Extracting deployment log from configLogDiv...'
  );

  if (!commitFrame) {
    throw new Error(
      'Commit result frame not found. Cannot extract deployment log.'
    );
  }
  ...
  try {
    await commitFrame.locator(
      ...
      console.log(
        'Show Log link click skipped or not required.'
      );
    });
    ...
    console.log(
      'Unable to click Show Log. Trying to extract configLogDiv directly.'
    );
  }

  const logContent =
    await commitFrame.evaluate(() => {
      const logDiv =
        document.querySelector('#configLogDiv');
  ...
  console.log(
    `Deployment log saved in logs folder: ${fileName}`
  );
...
async function waitForFinalDeploymentStatus(page) {
  console.log(
    'Waiting for final deployment status after Commit...'
  );
  ...
  for (let attempt = 1; attempt <= 60; attempt++) {
    ...
    writeLog(
      `deployment-final-status-attempt-${attempt}.txt`,
      fullText
    );

    if (!commitFrame) {
      console.log(
        `Commit status frame not found on attempt ${attempt}.`
      );
    ...
    const status =
      await getCommitStatus(commitFrame);

    console.log(
      `Deployment status attempt ${attempt}: ${status.id} - ${status.text}`
    );

    if (status.id === 'overallStatusInProgress') {
      await screenshot(
        page,
        `12-deployment-in-progress-${attempt}.png`
      );
    ...
    if (status.id === 'overallStatusComplete') {
      ...
      await screenshot(
        page,
        '13-deployment-complete.png'
      );

      const logText =
        await extractDeploymentLog(
          page,
          commitFrame,
      ...
      writeLog(
        '13-deployment-complete-page.txt',
        fullText
      );
    ...
    if (
      ...
      console.log(
        `Deployment failed or completed with errors: ${status.id}`
      );

      await screenshot(
        page,
        '13-deployment-failed-or-error.png'
      );
  ...
  await screenshot(
    page,
    '13-deployment-status-timeout.png'
  );

  writeLog(
    '13-deployment-status-timeout.txt',
    await getFullPageText(page)
  );

  throw new Error(
    'Timed out waiting for final deployment status.'
  );
}
...
async function clickBackToApplicationConfiguration(page, commitFrame) {
  console.log(
    'Clicking Back to Application Configuration...'
  );

  if (!commitFrame) {
    throw new Error(
      'Commit result frame not found. Cannot click Back to Application Configuration.'
    );
  }

  await screenshot(
    page,
    '14-before-back-to-application-configuration.png'
  );
  ...
  await screenshot(
    page,
    '15-after-back-to-application-configuration.png'
  );

  writeLog(
    '15-after-back-to-application-configuration.txt',
    await getFullPageText(page)
  );

  console.log(
    'Returned to Application Configuration page.'
  );
}
...
async function commitValidatedConfiguration(page, validationFrame) {
  ...
  if (!validationFrame) {
    ...
    throw new Error(
      'Validation frame not found. Cannot click Commit.'
    );

  }

  console.log(
    'Step 10: Clicking Commit button...'
  );

  await screenshot(
    page,
    '11-before-commit-click.png'
  );

  writeLog(
    '11-before-commit-click.txt',
    await getFullPageText(page)
  );
  ...
  console.log(
    'Commit button clicked. Waiting for commit/navigation to complete...'
  );
  ...
  await screenshot(
    page,
    '12-after-commit-wait.png'
  );

  writeLog(
    '12-after-commit-wait.txt',
    await getFullPageText(page)
  );

  await dumpPageDebugInfo(
    page,
    '12-after-commit-wait'
  );

  console.log(
    'Commit step completed and evidence captured.'
  );
}
...
console.log(
  'Step 9: Waiting for validation completion...'
);
...
console.log(
  'Step 10: Validation successful. Clicking Commit...'
);
...
console.log(
  'Commit button clicked. Waiting 60 seconds for navigation/commit processing...'
);
...
await screenshot(
  page,
  '12-after-commit-initial-wait.png'
);

writeLog(
  '12-after-commit-initial-wait.txt',
  await getFullPageText(page)
);

await dumpPageDebugInfo(
  page,
  '12-after-commit-initial-wait'
);

const deploymentResult =
  await waitForFinalDeploymentStatus(page);
